...
const Navbar = () => {
  const {
    customer, logout, language, setLanguage,
  ...
  return (
    <nav className="navbar">
      <Link to="/" className="navbar-brand">
        ...
        <div>
          <div>BankAI Platform</div>
          <div style={{ fontSize: '0.7rem', fontWeight: 400, color: 'var(--text-muted)', marginTop: -2 }}>
            {MODE_OPTIONS.find(m => m.value === mode)?.label || 'Branch Kiosk'}
          </div>
        </div>
      </Link>
